...
class Rig:
    ...
    def state(self):
        ...
        for attr, target, rate in (
            ...
            ("vram_t", gpu_target + 7, 0.10),
            ("vrm_t", 31 + cpu_load * 0.22, 0.08),
            ("board_t", 29 + (cpu_load + gpu_load) * 0.07, 0.05),
        ):
        ...
        sys_fan = curve(self.board_t, 30, 50, 20, 90)
        gpu_fan = 0 if self.gpu_t < 52 else curve(self.gpu_t, 52, 83, 30, 100)
        return {
            "cpu_load": cpu_load, "gpu_load": gpu_load,
            "cpu_t": self.cpu_t, "ccd_t": self.cpu_t + 2.5,
            "gpu_t": self.gpu_t, "hot_t": self.hot_t, "vram_t": self.vram_t,
            "vrm_t": self.vrm_t, "board_t": self.board_t,
            "chipset_t": self.board_t + 8,
            ...
            "ram": 38 + phase * 22, "vram": 20 + gpu_load * 0.5,
...
def build_tree() -> dict:
    ...
    mobo = {
        ...
        "Children": [{
            ...
            "Children": [
                _group("Temperatures", [
                    _sensor("/lpc/nct6687dr/0/temperature/0", "Temperature", "CPU Core", s["cpu_t"] - 1, "°C"),
                    _sensor("/lpc/nct6687dr/0/temperature/1", "Temperature", "System", s["board_t"], "°C"),
                    _sensor("/lpc/nct6687dr/0/temperature/2", "Temperature", "VRM MOS", s["vrm_t"], "°C"),
                    _sensor("/lpc/nct6687dr/0/temperature/3", "Temperature", "Chipset", s["chipset_t"], "°C"),
                ]),
                _group("Fans", [
                    ...
                    _sensor("/lpc/nct6687dr/0/fan/2", "Fan", "Chipset Fan", 0, "RPM", 0),
                    ...
                    _sensor("/lpc/nct6687dr/0/fan/14", "Fan", "System Fan #5", rpm(s["sys_fan"]), "RPM", 0),
                    _sensor("/lpc/nct6687dr/0/fan/15", "Fan", "System Fan #6", rpm(s["sys_fan"]), "RPM", 0),
                    _sensor("/lpc/nct6687dr/0/fan/3", "Fan", "EZ-Connect Fan", rpm(s["sys_fan"] + 10), "RPM", 0),
                ]),
                _group("Controls", [
                    _sensor("/lpc/nct6687dr/0/control/0", "Control", "CPU Fan", s["cpu_fan"], "%"),
                    _sensor("/lpc/nct6687dr/0/control/1", "Control", "Pump Fan", s["pump"], "%"),
                    _sensor("/lpc/nct6687dr/0/control/2", "Control", "Chipset Fan", 0, "%"),
                    ...
                    _sensor("/lpc/nct6687dr/0/control/14", "Control", "System Fan #5", s["sys_fan"], "%"),
                    _sensor("/lpc/nct6687dr/0/control/15", "Control", "System Fan #6", s["sys_fan"], "%"),
                    _sensor("/lpc/nct6687dr/0/control/3", "Control", "EZ-Connect Fan", s["sys_fan"] + 10, "%"),
                ]),
                _group("Voltages", [
    ...
    cpu = {
        ...
        "Children": [
            _group("Temperatures", [
                ...
                _sensor("/amdcpu/0/temperature/3", "Temperature", "CCD1 (Tdie)", s["ccd_t"], "°C"),
            ]),
            _group("Load", [
                _sensor("/amdcpu/0/load/0", "Load", "CPU Total", s["cpu_load"], "%"),
                _sensor("/amdcpu/0/load/1", "Load", "CPU Core Max", min(100, s["cpu_load"] * 1.7), "%"),
            ]),
            _group("Powers", [
    ...
    gpu = {
        ...
        "Children": [
            _group("Temperatures", [
                ...
                _sensor("/gpu-nvidia/0/temperature/3", "Temperature", "GPU Memory Junction", s["vram_t"], "°C"),
            ]),
            _group("Load", [
                _sensor("/gpu-nvidia/0/load/0", "Load", "GPU Core", s["gpu_load"], "%"),
                _sensor("/gpu-nvidia/0/load/1", "Load", "GPU Memory Controller", s["gpu_load"] * 0.6, "%"),
                _sensor("/gpu-nvidia/0/load/3", "Load", "GPU Memory", s["vram"], "%"),
            ]),
            _group("Controls", [
